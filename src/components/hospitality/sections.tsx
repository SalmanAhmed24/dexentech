...
/* ---------- Plugs into what you already use (53:8138) ---------- */
...
export function PlugsInto() {
  return (
    <section aria-labelledby="plugs-into" className="shell py-16 md:py-20">
      <Reveal>
        <h2
          id="plugs-into"
          className="text-[clamp(1.25rem,2.2vw,1.5rem)] font-semibold tracking-[-0.02em] text-ink-100"
        >
      ...
      <ul className="mt-6 flex flex-wrap gap-3">
        {PLUGS.map((plug, i) => (
          <Reveal as="li" key={plug} delay={i * 0.04} y={10}>
            <span className="block rounded-[10px] border border-[rgb(255_255_255/0.08)] bg-slate-900 px-[19px] py-[12px] text-[14px] text-ink-200 transition-colors duration-300 hover:border-[rgb(168_85_247/0.3)] hover:text-ink-100">
              {plug}
            </span>
          </Reveal>
        ))}
...
/* ---------- Priced to your property count (53:8154) ---------- */

export function PricingBand() {
  return (
    <section aria-labelledby="pricing-band" className="shell py-10 md:py-14">
      <Reveal>
        {/*
          Centred, per the design — a single dark card with a violet bloom
          bleeding down from the top edge, everything stacked on the axis.
        */}
        <div className="relative isolate overflow-hidden rounded-[18px] border border-[rgb(255_255_255/0.07)] bg-cinder px-6 py-16 text-center md:px-10 md:py-20">
          ...
          >
            Priced to your property count
          </h2>

          <p className="mx-auto mt-4 max-w-[480px] text-[15px] leading-6 text-pretty text-ink-400">
            Tell us your rooms and units. Get a number this week.
          </p>

          <div className="mt-8 flex justify-center">
            <Button href="/pricing">Get a Quote</Button>
          </div>
        </div>
      </Reveal>
    </section>
